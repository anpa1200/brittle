...
This document summarizes the locked [../PROTOCOL.md](../PROTOCOL.md). The canonical protocol is locked at `v1.0.0`; this summary is a convenience aid and must not override the canonical protocol.
...
The protocol defines one hypothesis and one null hypothesis per research question in [PROTOCOL.md Section 3](../PROTOCOL.md#3-hypotheses-phase-r3). The default minimum practical robustness-score difference is `0.10`.
...
The benchmark claim must be abandoned or downgraded if the final eligible corpus, mutation eligibility, evaluator repeatability, Sigma translation fidelity, auditability, or ethics checks fail the thresholds in [PROTOCOL.md Section 9.3](../PROTOCOL.md#93-falsification-criteria).
