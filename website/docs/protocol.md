...
Candidate public rule sources and later collection recipes are tracked separately in [Public Rule Source Inventory](./rule-source-inventory). That inventory is an input to Section 5.2 sampling design, not an authorization to collect corpus data before protocol lock.
...
> _See also: [Glossary](./glossary) for the full terminology table._
...
- Rules for which no acceptable ground-truth positive example can be obtained or constructed. This handling is resolved for Phase 1 by DEC-0006 in [DECISIONS.md](./decisions).
...
Operational definitions for each class, including edge cases and representative examples, must be finalized during the pre-lock pilot and committed to [DECISIONS.md](./decisions) before main execution. Once committed, these definitions are frozen for confirmatory Phase 1.
...
1. Snapshot public rule sources listed in [Public Rule Source Inventory](./rule-source-inventory), recording repository URL, retrieval method, commit hash or release tag, retrieval date, license, and local path.
...
- If a source must be re-downloaded before protocol lock, the new snapshot must replace the old one only through a logged decision in [DECISIONS.md](./decisions) and a protocol-change entry in [CHANGELOG.md](./changelog).
...
These confounders are expanded in [Threats to Validity](./threats-to-validity) during Phase R7.
...
The final pre-execution estimation precision check must simulate or bootstrap from the locked eligible-corpus structure before mutation generation begins. That check must be logged in [DECISIONS.md](./decisions) and must not use post-treatment robustness outcomes.
...
These mitigations reduce but do not eliminate internal validity risk. Remaining risks are summarized in Section 7 and expanded in [Threats to Validity](./threats-to-validity).
...
The detailed reproducibility checklist is maintained in [REPRODUCIBILITY.md](./reproducibility). If a step cannot be made fully reproducible because of LLM API drift, restricted samples, or licensing, the limitation must be disclosed in the final artifact report.

---

## 7. Threats to Validity (Phase R7)

> _This section is a summary. The full threat analysis is in [Threats to Validity](./threats-to-validity)._
...
Residual risk: some format, repository, or age strata may remain too small for strong inference. See [Threats to Validity](./threats-to-validity#r71-conclusion-validity-threats).
...
Residual risk: some semantic drift, evaluator mismatch, or reviewer bias may remain despite gating. See [Threats to Validity](./threats-to-validity#r72-internal-validity-threats).
...
Residual risk: readers may still treat the benchmark metric as a general security score unless the paper and dataset documentation keep the construct boundary explicit. See [Threats to Validity](./threats-to-validity#r73-construct-validity-threats).
...
Residual risk: the study cannot support claims about all detection content, private SOC rules, managed vendor detections, or future repository states. See [Threats to Validity](./threats-to-validity#r74-external-validity-threats).
...
Residual risk: even sanitized aggregate patterns may be misused. See [Threats to Validity](./threats-to-validity#r75-ethical-validity-threats).
...
Residual risk: exact replication may be impossible for restricted samples or changed LLM models. The target is auditable computational reproducibility for public artifacts plus conceptual replication where raw artifacts cannot be released. See [Threats to Validity](./threats-to-validity#r76-replication-threats).
...
4. If it appears to reveal a real vulnerability or dangerous operational method, handle it under [SECURITY.md](./security) rather than the benchmark release process.
...
This protocol is locked for confirmatory execution. No methodology change may be made without a new major protocol version and logged entries in [DECISIONS.md](./decisions) and [CHANGELOG.md](./changelog).
...
- Second reviewer recruited, trained on the coding guide, and committed to the `>=300`-decision sample, or an explicit downscoping plan is recorded in [DECISIONS.md](./decisions) that limits confirmatory N to what can be independently reviewed.
...
If a falsification criterion is triggered, the affected claim must be downgraded before publication. Possible downgrades are: pilot study, single-family benchmark, descriptive corpus audit, or exploratory methodology paper. Such a downgrade must be recorded in [DECISIONS.md](./decisions) and [CHANGELOG.md](./changelog).
